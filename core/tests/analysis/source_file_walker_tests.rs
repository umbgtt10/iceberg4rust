...
use iceberg4rust::analysis::source_file_walker::SourceFileWalker;
...
fn unique_temp_dir(label: &str) -> PathBuf {
    ...
    let dir = temp_dir().join(format!("file_risk_walker_{label}_{nanos}"));
...
fn walk_empty_directory_returns_empty() {
    ...
    // Act
    let files = SourceFileWalker::walk(&root);
